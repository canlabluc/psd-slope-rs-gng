...
#   regr_func: String, specifies which function to use when fitting to the PSD.
#              'ransac' or 'linreg'
#      lofreq: Scalar, specifies lower bound for regression
#      hifreq: Scalar, specifies upper bound for regression
#  export_dir: String, specifies directory to export results to

regr       = 'ransac'
lofreq     = 2
hifreq     = 24
export_dir = '/Users/jorge/Dropbox/research/_psd-slope/data/pipeline-full/'
...
def fit_slopes(subj, regr_func, lofreq, hifreq):
    ...
    subj['eyesC_slope'], subj['eyesC_fitline'] = regr_func(subj['f'], subj['eyesC_psd'], lofreq, hifreq)
    subj['eyesO_slope'], subj['eyesO_fitline'] = regr_func(subj['f'], subj['eyesO_psd'], lofreq, hifreq)
...
if regr == 'ransac':
    regr_func = ransac_slope
elif regr == 'linreg':
    regr_func = linreg_slope
...
subjoa = fit_slopes(subjoa, regr_func, lofreq, hifreq)
subjya = fit_slopes(subjya, regr_func, lofreq, hifreq)
...
# Export results
filename = export_dir + 'ya-oa-full-' + regr + '-' + str(lofreq) + '-' + str(hifreq) + '-eyesc-eyeso-sep.csv'

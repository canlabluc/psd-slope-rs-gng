...
def compute_subject_psds(import_path, import_path_csv, cut_recording_length, nwins_upperlimit=-1):
    ...
    for i in range(len(matfiles)):
        ...
        if cut_recording_length == True and len(subj[i]['data'][0]) > 215040:
            ...
            subj[i]['data'] = subj[i]['data'][:, 0:215040]
        ...
        subj[i]['data'] = len(subj[i]['data'][0]) # No longer needed, so clear it from memory
...
def main(argv):
    ...
    # fitting_func: 'linreg' or 'ransac', specifies which function to use for fitting.
    #               'linreg' is simple linear regression.
    #               'ransac' is RANSAC, a robust fitting method that ignores outliers.
    ...
    # nwins_upperlimit: Scalar, specifies the number of windows to extract
